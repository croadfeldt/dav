...
def _known_fields(cls, data: dict[str, Any], *, where: str) -> dict[str, Any]:
    """Filter `data` to dataclass `cls`'s declared fields, dropping + warning on unknown keys instead
    of letting `cls(**data)` raise TypeError. Authoring drift in a UC (e.g. an extra metadata key) must
    degrade to a warning, never hard-fail the whole UC at load. Operating-model DR §6 (tolerant loader)."""
