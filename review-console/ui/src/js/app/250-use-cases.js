...
async function loadUCs() {
  ...
  try {
    ...
    renderUCList();
    // Counts in the rail + the "All Use Cases (N UCs)" banner depend on allUCs — refresh whenever
    // UCs reload (the banner is otherwise only rendered by loadSets/selectSet, which can fire while
    // allUCs is still empty → the banner would stick at "0 UCs" even though the list shows N/N).
    renderSetFilterRail();
