...
function _runItemFingerprint(r) {
  return [
    ...
    r.run_id, _runModelLabel(r), r.quarantined,
    _rdName === r.name ? '1' : '0',
  ].join('|');
}
...
function _renderRunItemHtml(r) {
  ...
  return `
    ...
    <div class="rli-main" style="${r.archived ? 'opacity:0.55;' : ''}">
      ...
      <div class="rli-sub">${scopeBits.join(' · ')}${countsHtml ? ' · ' + countsHtml : ''}${refHtml ? ' · ' + refHtml : ''}${modelHtml ? ' · ' + modelHtml : ''}${quarHtml ? ' · ' + quarHtml : ''}</div>
