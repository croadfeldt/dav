...
_ENHANCEMENT_UC_SYSTEM = (
    ...
    "For EACH gap, emit one ENHANCEMENT block in this EXACT format. Every line "
    "MUST be flush-left (NO indentation) and every label MUST be plain text "
    "(no markdown bold, no `#` headings) — a line-oriented parser keys on them:\n\n"
...
_ENHANCEMENT_RUN_SYSTEM = (
    ...
    "For EACH enhancement use this EXACT block format. Every line MUST be "
    "flush-left (NO indentation) and every label MUST be plain text (no markdown "
    "bold, no `#` headings) — a line-oriented parser keys on them:\n\n"
    ...
    "After the blocks, emit a final ORDER line (flush-left) listing the "
    "enhancement IDs in the order a human should apply them (dependencies first):\n\n"
    "ORDER: <id1>, <id2>, <id3>, ...\n\n"
    "Rules:\n"
