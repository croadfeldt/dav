...
# errors at save time instead of at run time.
#
# SINGLE SOURCE OF TRUTH (operating-model DR §6): the DCM dimension vocab is owned by the engine's
# reference ConsumerProfile. The API build context can't import the engine, so the vocab is exported
# to `dcm_vocab.json` (generated by `python -m dav.scripts.export_dcm_vocab`; drift-guarded by its
# `--check` mode) and loaded here — NOT hand-duplicated. This is what prevented the
# single_gatekeeper/single_gating drift from recurring. Regenerate the JSON whenever the engine
# reference profile changes. When multi-consumer ships, swap this for a per-consumer profile API.
import json as _json
from pathlib import Path as _Path

def _load_dcm_vocab() -> dict:
    """Load the engine-exported DCM vocab artifact (ships in the app dir). Fail loud if missing —
    a missing vocab means validation would silently pass everything, which is worse than a clear error."""
    p = _Path(__file__).resolve().parent / "dcm_vocab.json"
    data = _json.loads(p.read_text())
    return {k: frozenset(v) for k, v in data.items() if isinstance(v, list)}
...
# Generation mode/source enums live on the UseCase schema (not the consumer profile), so they stay
# defined here; values mirror engine GenerationMode/GenerationSource.
